...
def run(config, results: List[CommandResult]):
    ...
    for n in range(12):
        expect(results[-1], f"Hello World {n}")


@matrix_command()
def cbuild(config):
    return ["cbuild", "-r", "--toolchain", config.compiler.toolchain, "--update-rte", "--packs", \
             "--context", f".Debug", "Hello.csolution.yml" ]


@matrix_command()
def model_exec(config):
    return ["FVP_Corstone_SSE-300", "-q", "--simlimit", 100, "-f", "model_config.txt",
            f"out/AVH/Hello.{config.compiler.image_ext}"]
